...
| Fonts / motion     | Inter Tight (display + body, weight carries the hierarchy) + JetBrains Mono via `next/font`; Framer Motion (landing) |
...
| ---------- | --------------------------------------------------------------------------------- | ------ |
| 2026-09-21 | **WhatsApp contact + "in-person classes" section; landing copy now says the portal is FREE for in-person students.** New `lib/whatsapp.ts` (the only place the number appears), `components/whatsapp-icon.tsx` (official mark, inlined), `components/landing/in-person.tsx`, a WhatsApp pill in the hero button row, and `NEXT_PUBLIC_WHATSAPP_NUMBER` documented in `.env.example`. Copy updated in `in-person.tsx`, `path-picker.tsx` and `how-it-works.tsx`. **No payment logic touched.** | **A floating scroll button was built first and then removed at the instructor's request** — worth recording, because the measurements are the reason the hero placement is better. A fixed corner button sits on top of whatever is in that corner: measured across the whole page it covered body text at **42 of 93 scroll positions at 375px** (15–16% at 768px and 1024px) and landed on four separate CTAs, including the in-person section's own WhatsApp button. Shrinking it to 48px moved that only from 42 to 37 — size is not the driver, position is. A yield-on-overlap behaviour was then built and abandoned with the float. The hero row has none of this: it is in the normal flow beside "Watch a Free Lesson", above the fold, and cannot cover anything. **Verified at 375px and 1280px:** all three pills 48px tall, same row on desktop, no overflow, no horizontal page scroll; `whitespace-nowrap` was needed because the label wrapped and overflowed the fixed-height pill in the narrow hero column. Icon is the official WhatsApp geometry from Simple Icons, inlined because `lucide-react` (our only icon dep) dropped brand icons. With the number unset both elements render nothing and the production build succeeds — checked, not assumed. ⚠️ **Known contradiction, deliberate:** `IN_PERSON_DISCOUNT_PERCENT` in `lib/money.ts` is still **40**, so an in-person account is charged 60% of list at checkout while the landing page now says the portal is free. The instructor asked for the copy change only and explicitly said to leave the payment logic alone. |
| 2026-09-20 | **Reading editor now confirms saves; markdown tables made phone-safe.** `updateItemBody` returns `{ ok, length }` instead of void, and the `RICH_TEXT` form in `builder.tsx` renders the result beside the Save button. `components/markdown.tsx` gains a `table` component override wrapping each table in its own `overflow-x-auto` container, plus `th`/`td` padding and `hr` spacing. | Reported as "paste markdown into a reading, click save, nothing happens". **It was saving the whole time** — the item in the live DB held 23,462 characters, timestamped to the reported attempt. The form rendered no feedback of any kind: the textarea kept the same text, the row did not change, so a successful save was indistinguishable from a dead button. Diagnosed by checking the data before touching any code, rather than assuming the report meant a failure. The table change is a consequence of the content that prompted this: real lecture notes carry 7- and 8-column trace tables, and `[&_table]:w-full` with no scroll container makes one wide table scroll the *entire page* sideways on a phone — the usual way long-form notes break. Tables are now `min-w-full` inside a scroller, so they exceed the container rather than being crushed. **Verified against the instructor's own 23KB document at 375px:** 9 tables, 9 scroll wrappers, widest table 513px on a 375px viewport, and `document.scrollWidth === clientWidth` — the table scrolls, the page does not. The item was temporarily flagged free-preview to view it, then restored to `isPublished=false, isFreePreview=false` with the body intact (an intermediate revert missed `isPublished` and was corrected). |
| 2026-09-19 | **Public free-lesson page (`/free-lesson`) + admin setting.** New nullable `Instructor.freeLessonVideoId / freeLessonTitle / freeLessonBlurb` (migration `free_lesson_video`, applied to the live DB), server action `admin/settings/actions.ts#setFreeLesson`, client form `free-lesson-form.tsx`, public page `app/free-lesson/page.tsx`. The three landing CTAs — hero button, navbar "Free Lesson", CTA-banner link — repointed from the `#try-a-lesson` anchor to the new route. Also extracted `youtubeEmbedUrl(id, { jsApi })` from `youtube.ts#getStreamUrl`. | The CTAs anchored to `FreeLessons`, a section that **returns null when no lesson is flagged `isFreePreview`** — so the most prominent button on the landing page scrolled nowhere and looked broken. Stored on `Instructor` beside the hero video rather than as a Video/LessonItem, for the same reason: marketing asset, no course, no gating, no progress. The link is parsed to a bare 11-char id **in the action**, so the DB only ever holds something the embed can be built from and a malformed link is rejected while the instructor is still looking at the form. `jsApi` is opt-in on the embed helper because it exists only for `youtube-player.tsx`'s progress tracking, which a page with no account has no use for. Page is `revalidate = 300` (same reasoning as landing/catalog) with `revalidatePath("/free-lesson")` on save, so an edit shows immediately instead of waiting out the window. Empty state points at the catalog rather than rendering a dead player. **Verified in-browser both ways:** empty state renders with the landing backdrop, and a temporarily-set id rendered the real embed with title/blurb from the DB — test row then cleared and the clear confirmed (all three columns null). Build shows `/free-lesson` as `○` prerendered. |
| 2026-09-19 | **Free-preview player fixed (`catalog/[courseId]/preview/[itemId]`).** Now branches on `videoProvider.playback` like the lesson page, rendering a plain iframe for YouTube. Also dropped the duplicated `— Cohort Portal` suffix from three pages' metadata. | Reported: marking a lesson "free preview" showed the old player with no video. The page rendered `<VideoPlayer>` **unconditionally**, so under `VIDEO_PROVIDER=youtube` it fed a YouTube *embed URL* into a `<video src>` and drew nothing — the YouTube work only ever updated the lesson page. Deliberately passes **no** `lessonItemId`: a preview is public, so there is no signed-in student whose progress could be recorded and beaconing it would write rows belonging to nobody. **Verified in-browser** against the real free-preview lesson (`_pBnNEAxwI4`), which now plays. The title fix is unrelated housekeeping found en route: the root layout applies a `"%s — Cohort Portal"` template, and three pages appended it again, so tabs read "Free preview — Cohort Portal — Cohort Portal". |
| 2026-09-19 | **Username login made case-insensitive (`lib/auth/login.ts`, `register/actions.ts`).** | Reported: only email worked at the login box. `authenticate()` lowercased the identifier for the email comparison but matched `username` against the raw trimmed input, so an account registered as "Megz" could never log in as "megz" — and nothing anywhere tells a student their username is case-sensitive while their email is not. Usernames still **store** the capitalisation chosen (admin screens read better for it); only the comparison changed. Registration's duplicate check became case-insensitive in the same commit, because allowing "Megz" and "megz" to coexist would make the login lookup ambiguous — it would resolve to whichever row came back first. **Verified against live data:** of 3 students exactly 1 has an uppercase letter in their username; the old exact-match query returned NOT FOUND for its lowercased form, the new one returns FOUND, and an exact-case lookup still matches (no regression). **Known gap, flagged not fixed:** the `@unique` index on `username` is case-SENSITIVE in Postgres, so the registration check is advisory against a concurrent double-signup. A `CREATE UNIQUE INDEX ... (LOWER(username))` would make it real — deliberately not added mid-launch, and negligible at 3 students since only the instructor registers accounts. |
| 2026-09-19 | **Landing page cached instead of `force-dynamic` (`app/page.tsx`).** Swapped `export const dynamic = "force-dynamic"` for `export const revalidate = 600`. One line; no component touched. | User reported the live site as very slow. Investigated rather than guessed, and the landing page was re-rendering server-side on **every** visit, making three DB round trips to eu-west-3 each time (`getHeroVideo`, `SaleBanner`, `FreeLessons`) — on the first page anyone sees. **Checked it was safe first:** nothing in the landing tree reads `cookies()`, `headers()` or `readSession()`, so the HTML is byte-identical for every visitor and cannot leak per-user state; the file comment records that this is a precondition, not an accident, so a future "Welcome back" navbar must revert it. **10 minutes chosen against the shortest-lived value on the page** — the hero video's signed URL is minted for 24h in `getHeroVideo()`, so any window well under that is safe and a near-24h cache would risk serving an expired link; the cost is that a sale or price edit takes up to 10 min to surface publicly. **Verified:** the build route table flipped `/` from `ƒ` (server-rendered on demand) to `○` (prerendered static content); page re-rendered clean afterwards with every request 200 and the testimonials present. (The 404s seen mid-verification were my own `pnpm build` overwriting `.next` under the running dev server — gone after a restart, not a code fault.) **Root cause of the slowness is NOT this and not the DB engine** — see the row below; the user had asked whether switching to MongoDB would help, and it would not: a multi-week rewrite that removes the joins this schema depends on. |
| 2026-09-19 | **Catalog cached too (`app/catalog/page.tsx`), `revalidate = 60`.** Same change as the landing page, shorter window. | Measured on the live deployment: `/catalog` took **1.70s** uncontended against **0.67s** for the now-cached landing page. Verified public first — neither the page, `PublicHeader` nor the root layout touches `cookies()`/`headers()`/session; ownership and lock state live on the course and lesson pages, which stay dynamic. **60s, not the landing page's 600s, because `liveWhere()` compares `publishAt` to *now*:** the cache window is the worst-case delay on a *scheduled* course or week going live, so a minute keeps that imperceptible while still collapsing a query-per-visitor into a query-per-minute. Build route table confirms `/catalog` flipped `ƒ` → `○`. |
| 2026-09-19 | **Found: `connection_limit=1` on the Render deployment (config, not code).** `DATABASE_URL` carries `?pgbouncer=true&connection_limit=1`, correct for Vercel (isolated function instances) and **actively harmful on Render**, which runs one long-lived Node server serving every user. Prisma therefore held a pool of exactly one connection for the whole site: every query in the app queued behind every other, so a 6-query page ran strictly serially and any two concurrent students blocked each other outright. Advised raising it to 10 in the Render dashboard — no deploy needed. | Recorded here because it is invisible in the repo: the value lives in `.env.render` and the host dashboard, so anyone reading the code alone would never find it, and it is the single largest contributor to the reported slowness. **Second suspect, unconfirmed at time of writing:** the database is in `aws-0-eu-west-3` (Paris) while Render's default region is Oregon — if the service was created on that default, every one of those serialized queries also crosses the Atlantic (~150ms each). Region cannot be changed on an existing Render service; it requires recreating it. **Also noted:** Render Starter is 0.5 CPU / 512 MB, thin for Next.js SSR — but the two items above are free to fix and were addressed first, hardware being the expensive answer to a configuration problem. |
| 2026-09-19 | **Logout redirect fixed for production (`api/auth/logout/route.ts`).** Replaced `NextResponse.redirect(new URL("/login?reason=loggedout", req.url))` with a plain 303 carrying a **relative** `Location: /login?reason=loggedout`. One line of behaviour, no other files touched. | First bug found on the live Render deployment: signing out landed the browser on `http://localhost:10000/login?reason=loggedout` → `ERR_CONNECTION_REFUSED`, and the user had to reload manually. **Root cause:** behind a proxy, `req.url` is the *internal* address the proxy dialled — Render's app port is 10000 — not the public URL, so `new URL(path, req.url)` resolved against the wrong origin and emitted an absolute redirect to a host only the server can reach. Invisible in development because there localhost genuinely *is* the public host. A relative `Location` is resolved by the browser against the URL it actually requested (RFC 7231 §7.1.2), so it is correct on localhost, on `*.onrender.com`, and on a custom domain later, with nothing to configure — strictly more robust than substituting `env.APP_URL`, which would silently bounce users off a custom domain if `APP_URL` ever drifted. **Verified:** against a local production-mode server the response is `303` with `location: /login?reason=loggedout` (relative, no host) and the session cookie expired in the same response; following the redirect ends at `/login?reason=loggedout` with `200`. `typecheck` clean. **Latent instance of the same bug left in place, flagged not fixed:** `middleware.ts:16` clones `req.nextUrl` for its HTTP→HTTPS redirect. It cannot fire on Render (the proxy sets `x-forwarded-proto: https`), but if a future front-end — Cloudflare, say — ever forwarded `http`, it would redirect to the internal host and take the whole site down rather than just logout. |
